...
void initMQTT() {
  // Set TLS configuration for secure connection
  espClientSecure.setInsecure(); // For simplified TLS (production should use proper certificates)
...
void connectMQTT() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str(), MQTT_USERNAME, MQTT_PASSWORD)) {
      ...
      // Subscribe to device control topics if needed
...
void handleMQTTConnection() {
  // Ensure NTP time is set for TLS
  static bool timeSet = false;
  if (!timeSet && WiFi.status() == WL_CONNECTED) {
    setDateTime();
    timeSet = true;
  }
  ...
  // Publish sensor data at specified intervals
  if (mqttConnected && (millis() - lastMqttPublish >= MQTT_PUBLISH_INTERVAL)) {
    publishSensorData();
...
void publishSensorData() {
  ...
  // Create JSON payload with current sensor data
  String payload = createMQTTPayload();
